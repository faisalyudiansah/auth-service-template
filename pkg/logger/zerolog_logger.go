...
func SetZerologLogger(cfg *config.Config) {
	...
	if cfg.App.Environment == constant.DEBUG {
...
func InjectToContext(ctx context.Context, log Logger) context.Context {
	if zl, ok := log.(*zerologLogger); ok {
		return zl.log.WithContext(ctx)
	}
	return ctx
}
